...
UPLOAD_FOLD = 'C:/Users/AKSHYA/AppData/Local/Programs/Python/Python37'
...
def upload_file():    
    ...
    if 'file' not in request.files:        
        ...
        return""    
    file = request.files['file']     # month = request.month['month']    
    if file.filename == '':        
        print('no selected file')        
        return""    
    if file :           
        for file in request.files.getlist('file'):
            data = pd.read_csv(file,encoding='windows-1252',parse_dates=['Order Date'])
        print(data.head())
        print("----------------------------------------------------------------------------------------")
        print(data.columns)
        print("----------------------------------------------------------------------------------------")
